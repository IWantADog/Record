...
- 在服务边界外使用https，在服务边界内部使用http。（默认内部服务的访问都是可信的）
- https的缺点
    - ssl之上的数据无法被代理服务器缓存。
    - https的加密解密会造成而外的开销。
- HMAC(Hash-based Message Authentication Code)

    使用HMAC，请求和私有密匙一起被哈希处理，生成的哈希值随请求一起发送。然后，服务器使用请求主体和自己的密匙副本重建哈希值。如果匹配则接受数据，

    这样做的好处是可以避免数据被篡改，而且私匙永远不会随请求发送，不存在私匙泄漏的问题。同时这种做法请求容易被缓存。

    不过这种做法只能避免数据被篡改，而不能避免数据被嗅探。

- 使用众所周知的加密算法，不要尝试自己实现加密算法。

## 康威定律和系统设计

康威定律：任何组织在设计一套系统（广义的系统）时，所交付的设计方案在结构上都与该组织的沟通结构保存一致。

- 对于微服务来说，开发人员需要对服务整个生命周期负责，从需求、构建、部署级运维。
    > 注意思想上的转变。
- 不管一开始看起来什么样，它永远都是人的问题。
- 请记住，如果没有把人们拉到一条船上，你想要的任何变化从一开始就注定会失败。

## 规模化微服务

- **假设一切都会失败，会让你从不同的角度去思考如何解决问题。**
- 了解你可以容忍多少故障，或者系统需要多块，**取决于系统的用户**。反过来，这会帮助你了解那些技术将对你有意义。也就是说，你的用户不是经常能阐明需求到底是什么，所以你需要通过问问题来提取正确的信息，并帮助他们了解不同级别的服务的相对成本。
- 合理的服务降级。考虑当下游的服务宕掉了会发生什么？
- 反脆弱的组织:
    - **超时**。如果使用等待太长时间来决定调用失败，整个系统都会被拖慢。如果超时太短，可能将正常调用错认为是失败的。如果完全没有超时，一个宕掉的下游服务可能会让整个系统挂起。给所有的跨进程调用设置超时，并选择一个默认超时时间。当超时发生后，记录在日志中，并相应的调整它们。
    - **断路器**。使用断路器，当对下游资源的调用发生一定数量的失败后，断路器会打开。接下来，所有的请求在断路器打开的状态下，会迅速的失败。一段时间后，客户端发送一些请求查看下游服务器是否已经恢复，如果它得到了正确的响应，则重置断路器。
    - **舱壁**。核心思想是将故障隔离起来，避免单个服务故障影响其他服务。
- 扩展数据库
    - 服务的可用性：服务始终可访问。
    - 数据的持久性：数据不会丢失。
    - 扩展读取。主从数据库，定期同步数据。
    ...
    - 扩展写入。数据库分片
        > 不过数据库分片会带来额外的查询复杂性。
    - CQRS(Command-Query Responsibility Segregation，命令查询职责分离)。系统的一部分负责获取修改状态的请求命令并处理它们。而另一部分负责处理查询。**这里的关键是处理命令和查询的模型本身是完全独立的。**
- 缓存
    - 客户端、代理和服务器缓存
    - http缓存:
        - cache-control
        ...
        - ETag
    - 为写操作使用缓存。适用于爆发式写入。
    - **保持简单**。避免在太多的地方使用缓存。使用的缓存越多，数据就越可能失效，就越难确定客户端最终看到是否是最新的数据。
- CAP定理

    在分布式系统中我们需要在C、A、P中权衡。具体的讲，就是我们最多只能实现其中的两个。

    - 一致性（consistency）: 当访问多个节点是能得到同样的数据。
    - 可用性（availability）: 每个请求都能得到响应。
    - 分区容忍性（partition tolerance）:当集群中的某些节点无法联系后，集群整体还能继续进行服务。
    - 整个的系统不必全是AP或是CP，应该根据实际的需要做出权衡。
