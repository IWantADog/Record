...
统计每个对象被引用的次数。如果被引用则加一，被删除则减一。当数量为0时，对象立即删除。

不过无法解决循环引用的问题（循环引用仅仅在容器对象中存在）。

### 标记删除

解决循环引用的问题。

通过外部引用，遍历所有容器对象（仅仅包含不可变对象的tuple & dict不需要跟踪），标记所有可达的对象。其余所有不可达的对象都会被清除。

### 分代回收

对象分为3代。

当对象被新创建时放在*generation 0*，当触发gc之后，如果对象未被清除将被移动到*generation 1*，会被更少的清除。

gc的触发时机通过对象声明和释放的数量差值决定，*generation 0 and 1*有对应各自的阈值。当差值大于阈值，则会触发对应代的清除逻辑。

对于*generation 2*使用*long_lived_pending / long_lived_total*确定是否需要执行检查（这里不深究具体字段的含义）。
