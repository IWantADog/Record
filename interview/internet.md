...
表现层: 将数据转换为能与接受者的系统格式兼容并适合传输的格式。
...
## 浏览器DNS的域名解析过程
1. 查询本地缓存。浏览器查询缓存中是否存储过该域名的IP地址。
2. 查询系统的DNS缓存。
3. 向本地的DNS服务器发送请求。操作系统向网络配置的DNS服务器发起DNS查询。这里的本地DNS服务器如果不主动配置，一般使用的是ISP提供的DNS服务器地址。
4. 如果本地DNS服务器中也没有。则会开始递归查询。依次向根DNS服务器（.）、顶级服务器(com)、权威DNS服务器地址（google.com）发起请求。最终权威DNS服务器会找到对应的IP地址。
5. 浏览器获取到IP地址。
