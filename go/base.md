...
结构体能够直接调用其内嵌类型结构体的方法。不过需要注意内嵌并不等同于面向对象中的继承。
...
TODO:
当main函数返回时，所有的goroutine都暴力地被终结。
...
select一直等待，直到一次通信来告知一些情况可以执行。然后，它进行这次通信，执行该情况所对应的代码，其他的通信将不会发生。对于没有对应情况的select，select{}将永远等待。而如果多种情况同时满足，select就随机选择一个，这样保证每一个通道有相同的机会被选中。
...
避免数据竞争的方法：
1. 不要修改变量。
2. 避免从多个goroutine访问同一个变量。将对变量的修改限制在一个goroutine中。使用通道请求来代理一个受限变量的所有访问的goroutine称为该变量的监控goroutine。
